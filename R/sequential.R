...
if ((nrow(data) == ncol(data)) == FALSE) {
  ...
  if ((adjacent == 0) & (any(diag(freqs) == 0))) {
    warning("You have indicated that consecutive or adjacent codes can never repeat (adjacent = 0), 
             yet repeating codes have been found in the data. See the main diagonal of the 
             frequency matrix. This specification error will result in faulty computations for LRX2, 
          ...
          for (i in 2:(nrow(datap) - 2)) {
            ...
            for (j in 1:limit) {
              ...
              if ( (datap[i - 1] != datap[kay]) & (datap[i + 1] != datap[kay]) & 
                   (datap[kay - 1] != datap[i]) & (datap[kay + 1] != datap[i]) ) { break
              }
            }
            d <- datap[i]
            datap[i] <- datap[kay]
...
seqOutput <- list(freqs=freqs, expfreqs=expfreq, probs=tprob, chi=tlr, adjres=zadjres,
                   p=pzadjres, YulesQ=yulesq, kappas=kappa, z=zkappa, pk=pzkappa)

return(invisible(seqOutput))
