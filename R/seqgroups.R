...
# This program computes a variety of basic sequential analysis statistics for more than one group/segment. The data are assumed to be a series of integer codes with values ranging from "1" to whatever value the user specifies in the "ncodes" computation at the start of the program.

seqgroups <- function(alldata = NULL, ncodes = 6, labels = NULL, lag = 1, adjacent = 1,
  onezero = NULL, tailed = 2, test = 1, output = 2, outfile = 1) {
	...
	if (is.null(alldata)) {
		# Enter/read the data into a matrix with the name "alldata", with code values > 999 indicating different groups or segments.

		alldata <- rbind(1000, 2, 5, 2, 4, 2, 5, 2, 4, 2, 4, 2, 2, 5, 2, 4, 2, 5, 3,
			4, 3, 4, 1, 4, 4, 1, 2, 5, 2, 1, 5, 4, 2, 5, 1, 4, 4, 2, 3, 6, 6, 2, 2,
			6, 5, 5, 5, 6, 3, 6, 3, 3, 6, 3, 2, 5, 3, 5, 2, 6, 2, 2, 2, 5, 2, 4, 4,
			5, 2, 5, 2, 5, 5, 4, 2, 5, 5, 5, 2, 5, 2, 5, 2, 5, 2, 2, 5, 2, 5, 2, 2,
			5, 5, 2, 2, 2, 5, 2, 5, 2, 5, 2, 5, 2, 2, 5, 2, 5, 3, 5, 2, 5, 5, 2, 2,
			5, 2, 2, 5, 2, 6, 2, 5, 4, 2, 5, 2, 4, 4, 2, 1, 2, 2000, 5, 2, 5, 2, 5,
			5, 5, 3, 5, 2, 5, 5, 2, 5, 5, 2, 5, 2, 5, 2, 4, 2, 4, 2, 5, 4, 2, 2, 5,
			2, 5, 5, 5, 2, 5, 2, 5, 5, 2, 5, 2, 5, 5, 2, 1, 2, 5, 1, 5, 1, 5, 5, 4,
			2, 2, 2, 3, 6, 3, 6, 3, 6, 3, 6, 3, 6, 3, 3, 6, 6, 6, 5, 1, 2, 5, 2, 5,
			5, 2, 4, 5, 5, 2, 5, 5, 2, 5, 2, 5, 2, 5, 1, 5, 4, 2, 2, 5, 2, 5, 2, 5,
			4, 1, 4, 4, 2, 4, 4, 2, 4, 2, 3, 5, 4, 2, 2, 5, 2, 6, 1, 4, 1, 4, 5, 5,
			5, 4, 5, 2, 4, 5, 3000, 5, 2, 5, 2, 2, 2, 5, 2, 2, 5, 2, 2, 4, 2, 2, 2,
			6, 2, 4, 3, 3, 3, 3, 2, 3, 6, 3, 5, 3, 3, 5, 3, 2, 2, 2, 2, 6, 3, 6, 3,
			6, 2, 6, 1, 6, 3, 3, 3, 3, 5, 3, 5, 2, 5, 3, 3, 6, 3, 1, 4, 1, 5, 1, 5,
			6, 3, 3, 6, 3, 6, 3, 6, 3, 6, 3, 6, 3, 3, 6, 3, 6, 3, 2, 5, 2, 5, 2, 5,
			2, 5, 2, 2, 2, 3, 5, 1, 6, 1, 6, 6, 6, 1, 6, 6, 1, 6, 1, 6, 2, 5, 1, 6,
			6, 6, 2, 2, 6, 6, 6, 2, 6, 2, 6, 2, 5, 5, 5, 5, 5, 5, 6)
	}

	alldata <- as.matrix(alldata, ncol = 1)
	...
	for (dindex in 1:nrow(each)) {
		...
		for (i in 1:ncodes) {
			for (j in 1:ncodes) {
				...
				# Note: more refined computations for when adjacent codes cannot repeat appear below, after the above 2 loops are complete.
      ...
      if (is.null(onezero)) {
        ...
        diag(onezero) <- 0
			...
			} else cat("did NOT converge after the following number of iterations:", ipfloop,
				"\n\n")
		...
		if ((dindex < nrow(each)) & (output == 2)) {
			...
			colnames(lchisq) <- c("LRX2", "df", "sig.")
			print(round(lchisq,2))
		}
		...
		if ((dindex == nrow(each)) || (output == 2)) {
			...
			if (adjacent == 1) {
				dft <- (ncodes - 1)^2
			} else dft <- (ncodes - 1)^2 - (ncodes^2 - sum(onezero))
...
seqgOutput <- list(freqs=freqs, expfreqs=expfreq, probs=tprob, chi=lchisq, adjres=zadjres,
                   p=pzadjres, YulesQ=yulesq, kappas=kappa, z=zkappa, pk=pzkappa)

return(invisible(seqgOutput))
